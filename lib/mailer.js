...
export function asuntoResumenActividades({ filas, filasForm }) {
  return `📊 Resumen semanal · ${(filas || []).length} actividades y ${(filasForm || []).length} formularios`;
}
export function plantillaResumenActividades({ desde, hasta, filas, filasForm }) {
  filas = filas || [];
  filasForm = filasForm || [];
  ...
  const rows = filas.map((r) =>
    `<tr><td style="padding:5px 8px;border-bottom:1px solid #1e3a58;font-size:13px;white-space:nowrap">${r.fecha || ''}</td>
     <td style="padding:5px 8px;border-bottom:1px solid #1e3a58;font-size:13px;white-space:nowrap">${r.nombre || ''}</td>
     <td style="padding:5px 8px;border-bottom:1px solid #1e3a58;font-size:13px;white-space:nowrap">${r.curso || ''}${r.edicion ? ' · ' + r.edicion : ''}</td>
     <td style="padding:5px 8px;border-bottom:1px solid #1e3a58;font-size:13px;white-space:nowrap">${r.actividad || ''}</td>
     <td style="padding:5px 8px;border-bottom:1px solid #1e3a58;font-size:13px;text-align:right;font-weight:700;white-space:nowrap">${r.puntaje}/${r.total}</td></tr>`
  ).join('');
  ...
  return `${WRAP_OPEN}
    ...
    <tr><td style="padding:24px 26px;color:#e7eef6;font-family:Arial,sans-serif;font-size:14px;line-height:1.5">
      ...
      ${filasForm.length ? `<div style="font-family:'Jost',Arial,sans-serif;font-size:15px;font-weight:700;color:#fff;margin:22px 0 8px">📝 Formularios completados (${filasForm.length})</div>
      <table role="presentation" cellpadding="0" cellspacing="0" style="width:100%;background:#12233b;border:1px solid #1e3a58;border-radius:10px;border-collapse:collapse">
        <tr><td style="padding:6px 8px;color:#96aac4;font-size:12px">Fecha</td><td style="padding:6px 8px;color:#96aac4;font-size:12px">Persona</td><td style="padding:6px 8px;color:#96aac4;font-size:12px">Formulario</td><td style="padding:6px 8px;color:#96aac4;font-size:12px">Curso</td></tr>
        ${filasForm.map((r) => `<tr><td style="padding:5px 8px;border-bottom:1px solid #1e3a58;font-size:13px">${r.fecha || ''}</td><td style="padding:5px 8px;border-bottom:1px solid #1e3a58;font-size:13px">${r.nombre || ''}</td><td style="padding:5px 8px;border-bottom:1px solid #1e3a58;font-size:13px">${r.formulario || ''}</td><td style="padding:5px 8px;border-bottom:1px solid #1e3a58;font-size:13px">${r.curso || ''}</td></tr>`).join('')}
      </table>` : ''}
...
export async function enviarResumenActividades({ destinatarios, desde, hasta, filas, filasForm }) {
  const transporter = getTransporter();
  const payload = { desde, hasta, filas, filasForm };
  await enviarYlog(transporter, 'Resumen viernes', {
    from: `Plataforma ILCE <${process.env.GMAIL_SENDER_EMAIL}>`,
    to: destinatarios.join(', '),
    subject: asuntoResumenActividades({ filas, filasForm }),
    html: plantillaResumenActividades(payload),
    text: `Resumen semanal ${desde} a ${hasta}: ${(filas || []).length} actividad(es) y ${(filasForm || []).length} formulario(s).`
  }, payload);
}
