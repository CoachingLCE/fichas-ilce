'use client';
// Recorrido guiado ("❓ Necesito ayuda"), adaptado del que ya usa Presentismo ILCE. La
// diferencia principal es que acá no hay rutas por página: todo vive en /panel con una
// pestaña activa (`tab`), así que en vez de navegar a una URL, un paso "cambia de pestaña"
// llamando a setTab.
import { useCallback, useEffect, useMemo, useState } from 'react';
import { TOUR_PASOS, TAREAS_AYUDA } from '../lib/tourSteps';

export default function TourGuiado({ tab, setTab, permisos }) {
  ...
  // Si el paso vive en otra pestaña, cambiamos de pestaña; si no, buscamos el elemento en
  // el DOM (con reintentos cortos, porque la pestaña recién montada puede tardar un
  // instante en renderizar el contenido real, por ejemplo mientras llegan los datos).
  useEffect(() => {
    if (!activo || !pasoActual) return;
    if (pasoActual.tab && pasoActual.tab !== tab) {
      setBuscando(true);
      setTab(pasoActual.tab);
      return;
    }
    setBuscando(false);
    let intentos = 0;
  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-14 right-4 z-[90] bg-gradient-to-r from-accentPurple to-accentMagenta text-white text-sm font-semibold px-4 py-2.5 rounded-full shadow-lg flex items-center gap-1.5 hover:opacity-90 transition-opacity"
      >
        ❓ Necesito ayuda
      </button>
      ...
      {menuAbierto && !activo && (
        <div className="fixed inset-0 z-[91] flex items-end justify-end p-5" onClick={() => setMenuAbierto(false)}>
          <div className="bg-surface2 border border-border rounded-2xl p-4 w-80 max-w-[calc(100vw-40px)] shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-sm font-semibold mb-1">Te mostramos cómo funciona Fichas ILCE</h3>
            <p className="text-xs text-textSec mb-3">Vamos a recorrer juntos las principales secciones del panel.</p>
            <button
...
function TourOverlay({ paso, idx, total, rect, buscando, modoTarea, onSiguiente, onAnterior, onSalir }) {
  ...
  const tooltipStyle = useMemo(() => {
    ...
    // En el celular el tooltip fijo de 320px se pasaba del ancho de la pantalla en equipos
    // angostos (<348px) — acá se achica para no generar scroll horizontal.
