...
function calcularResumenGlobal(todosLosEstudiantes, todasLasEdiciones, cursosInfo) {
  ...
  todosLosEstudiantes.forEach((e) => {
    // .trim(): si "Edicion" llega con un espacio de más (pegado desde afuera, por ejemplo), sin esto
    // se arma una clave distinta a la que después busca guardarFormadorEdicion/la fila de
    // AcademicoEdiciones (que sí llega recortada desde el servidor) — el Formador se guarda bien,
    // pero nunca se vuelve a encontrar al releer, y da la sensación de que "no se guardó".
    const ed = (e.Edicion || '').trim() || 'Sin edición';
    const curso = (e.Curso || '').trim();
    const clave = `${curso}|||${ed}`;
    if (!porClave[clave]) {
      porClave[clave] = { curso, edicion: ed, inscritos: 0, certificados: 0, bajas: 0, cc: 0, noCertificaron: 0, pendientes: 0 };
    }
  ...
  return Object.values(porClave).map((r) => {
    const edicionInfo = todasLasEdiciones.find(
      (x) => (x.Curso || '').trim() === r.curso && (x.Edicion || '').trim() === r.edicion
    );
...
export default function AcademicoPage() {
  ...
  const resumenOrdenado = [...resumenFiltrado].sort((a, b) => {
    // OJO: FechaInicio puede venir en ISO (AAAA-MM-DD, lo que guarda el <input type="date">) o en
    // "DD/MM/AAAA" (filas cargadas a mano/pegadas). new Date(...) directo interpreta mal ese segundo
    // formato (lo lee como MM/DD/AAAA) y da fechas inválidas o corridas — por eso antes el orden
    // "de más nueva a más vieja" salía mezclado. parsearFechaFlexible entiende los dos formatos.
  ...
  const serieCronologica = [...resumenPorDocente]
    ...
    .sort((a, b) => parsearFechaFlexible(a.fechaInicio) - parsearFechaFlexible(b.fechaInicio))
