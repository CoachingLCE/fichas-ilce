...
export default function InscritosPage() {
  ...
  // El cartel de "acciones pendientes" se puede cerrar por hoy con la cruz — vuelve a aparecer
  // solo al otro día (se guarda la fecha en la que se cerró, no un booleano fijo, para que no
  // quede cerrado para siempre). Es por navegador/persona, no algo que se comparta con el equipo.
  useEffect(() => {
    try {
      const hoy = new Date().toISOString().slice(0, 10);
      setBannerCerrado(localStorage.getItem('inscritos-banner-pendientes-cerrado') === hoy);
    } catch {}
  }, []);

  function cerrarBannerPorHoy() {
    try {
      const hoy = new Date().toISOString().slice(0, 10);
      localStorage.setItem('inscritos-banner-pendientes-cerrado', hoy);
    } catch {}
  ...
  async function reenviarAlta(inscrito) {
    ...
    try {
      ...
      mostrarToast('No se pudo conectar con el servidor — probá de nuevo.');
      return;
    }
    setEnviandoAltaId(null);
    // Antes esto se daba por exitoso siempre (aunque el pedido fallara), así que si algo salía
    // mal el estudiante se quedaba "pegado" en la alerta sin ningún aviso de que en realidad no
    // se había reenviado nada — como si el click no hubiera hecho nada.
    if (res.ok) {
      mostrarToast('Alta reenviada');
      cargarInscritos(true);
    } else {
      const r = await res.json().catch(() => ({}));
      mostrarToast(r.error || 'No se pudo reenviar el alta — probá de nuevo.');
    }
  }
    ...
    <div>
      ...
      <div className="max-w-[1900px] mx-auto px-4 pb-16">
        {(() => {
          ...
          if (totalAcciones === 0 || bannerCerrado) return null;
          return (
            <div className="relative bg-warningBg border border-warningText/30 rounded-xl px-4 py-2.5 pr-9 mb-4 no-print">
              <button
                onClick={cerrarBannerPorHoy}
                title="Cerrar por hoy — vuelve a aparecer mañana si sigue habiendo pendientes"
                className="absolute top-2 right-2 text-warningText/70 hover:text-warningText text-sm leading-none w-5 h-5 flex items-center justify-center rounded hover:bg-warningText/10"
              >
                ✕
              </button>
